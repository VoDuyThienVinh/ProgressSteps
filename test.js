...
nextBtn.addEventListener('click', () => {
  ...
  // circleEl[countActive].classList.add('active');
  updateDOM();
})
...
prevBtn.addEventListener('click', () => {
  ...
  // circleEl[countActive+1].classList.remove('active');
  updateDOM();
})
...
function updateDOM() {
  let progressPercent = ((countActive)/(lengthCircleEl-1)) * 100;
  progressEl.style.width=`${progressPercent}%`;
  circleEl.forEach((circle, id) => { // index 0 -> 3 // important countActive progressBar and circleElement
    if(id <= countActive) {
